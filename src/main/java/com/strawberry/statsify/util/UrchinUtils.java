...
public class UrchinUtils {
    ...
    private static String getFormattedTags(String username, UrchinApi urchinApi, String urchinKey) throws IOException {
        return urchinApi
    ...
    private static void handleException(String username, Exception e) {
        printMessage("§r[§bF§r] Failed to fetch tags for " + username + " | " + e.getMessage());
    }

    public static void checkAndPrintUrchinTags(String username, UrchinApi urchinApi, String urchinKey, boolean withPlayerName) {
        try {
            String tags = getFormattedTags(username, urchinApi, urchinKey);
            if (!tags.isEmpty()) {
                String message;
                if (withPlayerName) {
                    message = "§r[§bF§r] §c⚠ §r" + PlayerUtils.getTabDisplayName(username) + " §ris §ctagged§r for: " + tags;
                } else {
