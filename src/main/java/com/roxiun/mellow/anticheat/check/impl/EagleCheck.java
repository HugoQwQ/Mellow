...
public class EagleCheck extends Check {
    ...
    public void onPlayerTick(
        ...
        double moveYaw = getRelativeMoveAngle(deltaX, deltaZ, player.rotationYaw);
        ...
        if (lookingDown && holdingBlock) {
            ...
            if (quickCrouch && swingOnCrouch) {
                boolean hasConsistentPattern =
                        data.crouchDurations.size() >= 3
                                && data.crouchDurations.stream().limit(3).allMatch(d -> d <= 2);

                double consistencyScore = 0.0;
                if (data.crouchDurations.size() >= 3) {
                    double mean =
                            data.crouchDurations.stream()
                                    .limit(3)
                    ...
                    double variance =
                            data.crouchDurations.stream()
                                    ...
                                    .average()
                                    .orElse(0.0);
                    consistencyScore = Math.max(0.0, Math.min(1.0, 1.0 - (variance / 4.0)));
                }

                double vlAmount = 2.0;
                double pitchMultiplier = extremelyLookingDown ? 1.5 : 1.0;
                double swingTimingMultiplier =
                        data.lastSwingTick == data.lastCrouchEndTick ? 1.4 : 1.0;
                double movementMultiplier =
                        movingDirectlyBackwards ? 1.8 : (movingBackwards ? 1.5 : 1.0);
                ...
                if (data.eaglePatternCount >= 2) {
                    ...
                    manager.flag(
                            ...
                            String.format(
                                    ...
                                    checkType, moveYaw, crouchDuration, consistencyScore),
                            finalVL);
                    flagged = true;
                    data.eaglePatternCount = 0;
                }
            ...
            else if (data.lastSwingTick == tick
                    && data.lastCrouchEndTick >= tick - 1
                    && data.lastCrouchStartTick == data.lastCrouchEndTick - 1) {
                double pitchMultiplier =
                        extremelyLookingDown ? 1.6 : (player.rotationPitch >= 80f ? 1.3 : 1.0);
                double movementMultiplier =
                        movingDirectlyBackwards ? 1.5 : (movingBackwards ? 1.2 : 1.0);

                data.eagleConsecutiveViolations++;
                double consecutiveMultiplier =
                        1.0 + (Math.min(data.eagleConsecutiveViolations, 5) * 0.15);
                double vlAmount =
                        3.5 * pitchMultiplier * movementMultiplier * consecutiveMultiplier;
